...
class SessionPersistenceManager:
    ...
    def save_session(self, session_id: str, name: str, query: str, segmento: str, 
                    openrouter_model: str = None, produto: str = None, publico: str = None,
                    preco: str = None, objetivo_receita: str = None, contexto_adicional: str = None) -> Dict[str, Any]:
    ...
    def find_similar_session(self, segmento: str, produto: str = None, publico: str = None) -> Optional[Dict[str, Any]]:
        """Busca sessão similar baseada em segmento, produto e público"""
        try:
            sessions = self.get_sessions()
            logger.info(f"🔍 Buscando sessão similar para: segmento='{segmento}', produto='{produto}', publico='{publico}'")
            logger.info(f"🔍 Total de sessões disponíveis: {len(sessions)}")
            
            # Primeiro, busca por correspondência exata de segmento + produto + público
            for session in sessions:
                session_segmento = session.get('segmento', '').lower().strip()
                session_produto = session.get('produto', '').lower().strip()
                session_publico = session.get('publico', '').lower().strip()
                
                logger.info(f"🔍 Verificando sessão {session.get('id')}: segmento='{session_segmento}', produto='{session_produto}', publico='{session_publico}'")
                
                # Verifica se segmento é igual
                if session_segmento == segmento.lower().strip():
                    # Se produto e público foram fornecidos, verifica ambos
                    if produto and publico:
                        if (session_produto and produto.lower().strip() in session_produto) and \
                           (session_publico and publico.lower().strip() in session_publico):
                            logger.info(f"🔍 Sessão similar encontrada por segmento+produto+público: {session.get('id')}")
                            return session
                    
                    # Se apenas produto foi fornecido
                    elif produto and not publico:
                        if session_produto and produto.lower().strip() in session_produto:
                            logger.info(f"🔍 Sessão similar encontrada por segmento+produto: {session.get('id')}")
                            return session
                    
                    # Se apenas público foi fornecido
                    elif publico and not produto:
                        if session_publico and publico.lower().strip() in session_publico:
                            logger.info(f"🔍 Sessão similar encontrada por segmento+público: {session.get('id')}")
                            return session
                    
                    # Se nem produto nem público foram fornecidos, retorna a primeira com mesmo segmento
                    elif not produto and not publico:
                        logger.info(f"🔍 Sessão similar encontrada por segmento: {session.get('id')}")
                        return session
            
            # Segunda tentativa: busca apenas por segmento (mais flexível)
            for session in sessions:
                session_segmento = session.get('segmento', '').lower().strip()
                if session_segmento == segmento.lower().strip():
                    logger.info(f"🔍 Sessão similar encontrada (fallback por segmento): {session.get('id')}")
                    return session
            
            logger.info(f"🔍 Nenhuma sessão similar encontrada para segmento: {segmento}")
            return None
            
        except Exception as e:
            logger.error(f"❌ Erro ao buscar sessão similar: {e}")
            return None
