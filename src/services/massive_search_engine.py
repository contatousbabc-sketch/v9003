...
class MassiveSearchEngine:
    ...
    async def execute_massive_search(self, produto: str, publico_alvo: str, session_id: str, **kwargs) -> Dict[str, Any]:
        ...
        TIME_LIMIT_SECONDS = 8 * 60  # 8 minutos - otimizado para melhor cobertura
    ...
    async def _search_alibaba_websailor(self, query: str, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # ✅ CORRIGIDO: Conta o texto extraído REAL das fontes
            texto_extraido = 0
            if navigation_result and isinstance(navigation_result, dict):
                conteudo = navigation_result.get('conteudo_consolidado', {})
                if conteudo:
                    # Conta texto das fontes detalhadas (onde está o conteúdo real)
                    fontes = conteudo.get('fontes_detalhadas', [])
                    for fonte in fontes:
                        if isinstance(fonte, dict):
                            # Conta conteúdo real extraído
                            conteudo_real = fonte.get('conteudo_real', '')
                            snippet_real = fonte.get('snippet_real', '')
                            
                            if conteudo_real:
                                texto_extraido += len(str(conteudo_real))
                            elif snippet_real:
                                texto_extraido += len(str(snippet_real))
                    
                    # Fallback: conta insights e tendências se não há fontes
                    if texto_extraido == 0:
                        ...
                        texto_extraido += sum(len(str(item)) for item in insights)
                        texto_extraido += sum(len(str(item)) for item in tendencias)
                        texto_extraido += sum(len(str(item)) for item in oportunidades)
