...
# -*- coding: utf-8 -*-
"""
ARQV18 Enhanced v18.0 - Viral Content Analyzer
...
class ViralContentAnalyzer:
    ...
    async def _analyze_basic_content(self, url: str, content: Dict) -> Optional[Dict]:
        """Análise básica de conteúdo para qualquer plataforma quando métodos específicos falham"""
        try:
            ...
            full_text = f"{title} {description} {snippet}".strip()
            
            # Análise básica do conteúdo textual
            hashtags = self._extract_hashtags(full_text)
            mentions = self._extract_mentions(full_text)
            
            # Tenta extrair métricas básicas do texto
            likes = self._extract_number_from_text(full_text, ['likes', 'curtidas', 'gostei', 'like'])
            comments = self._extract_number_from_text(full_text, ['comments', 'comentários', 'respostas', 'comment'])
            views = self._extract_number_from_text(full_text, ['views', 'visualizações', 'assistiram', 'view'])
            shares = self._extract_number_from_text(full_text, ['shares', 'compartilhamentos', 'share'])
            ...
            analysis = {
                ...
                'hashtags': hashtags,
                'mentions': mentions,
                'is_video': self._detect_video_content(full_text, url),
                'engagement_rate': 0,
                'viral_score': 0,
                'analysis_timestamp': datetime.now().isoformat(),
                ...
                'has_media': self._detect_media_content(full_text, url)
            }
            
            # Calcular viral score baseado nos dados disponíveis
            analysis['viral_score'] = self._calculate_viral_score(analysis, platform)
            
            # Calcular engagement rate estimado
            if views > 0:
                analysis['engagement_rate'] = (likes + comments + shares) / views
            elif likes > 0:
                analysis['engagement_rate'] = comments / likes if likes > 0 else 0
            
            logger.info(f"✅ Análise básica concluída para {platform}: {likes} likes, {comments} comentários")
            return analysis
            
        except Exception as e:
            logger.error(f"❌ Erro na análise básica de conteúdo {url}: {e}")
            return None
    ...
    def _detect_video_content(self, text: str, url: str) -> bool:
        """Detecta se o conteúdo é um vídeo"""
        video_indicators = ['video', 'vídeo', 'watch', 'play', 'reel', 'story', 'clip']
        text_lower = text.lower()
        url_lower = url.lower()
        
        # Verifica indicadores no texto
        for indicator in video_indicators:
            if indicator in text_lower:
                return True
        
        # Verifica indicadores na URL
        video_url_patterns = ['/reel/', '/video/', '/watch', '/v/', 'youtu.be', 'tiktok.com']
    ...
    def _detect_media_content(self, text: str, url: str) -> bool:
        """Detecta se há conteúdo de mídia (foto/vídeo)"""
        media_indicators = ['photo', 'foto', 'image', 'imagem', 'video', 'vídeo', 'pic', 'picture']
        text_lower = text.lower()
        ...
        # URLs de plataformas de mídia social geralmente têm mídia
        media_platforms = ['instagram.com', 'tiktok.com', 'youtube.com', 'youtu.be']
        url_lower = url.lower()
