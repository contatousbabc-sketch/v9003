...
def save_session():
    ...
    try:
        ...
        if not session_id:
            # Primeiro, tenta encontrar uma sessão similar baseada em segmento, produto e público
            segmento = data.get('segmento', '')
            produto = data.get('produto', '')
            publico = data.get('publico', '')
            
            similar_session = session_persistence.find_similar_session(
                segmento=segmento,
                produto=produto if produto else None,
                publico=publico if publico else None
            )
            
            if similar_session:
                session_id = similar_session.get('id')
                logger.info(f"🔄 Reutilizando sessão similar: {session_id}")
            else:
                # Se não encontrou sessão similar, cria nova
                import uuid
                import time
